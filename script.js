...
// 🪐 FUNÇÃO DO NOVO ALERTA ESPACIAL ESTILIZADO
...
function desenharPainelDeGrupos() {
    ...
    if(window.twemoji) { twemoji.parse(grid); }
}
...
function selecionarPosicaoTime(letra, time, idx) {
    ...
    if (lista.includes(time)) {
        ...
        if (lista.length >= 2) { 
            // 🔄 Atualizado para o Alerta Customizado
            mostrarAlertaCosmico(`O Grupo ${letra} já possui os dois classificados definidos! Desmarque uma seleção para poder alterar.`); 
            return; 
        }
        lista.push(time);
    }
...
function salvarPalpitesGrupoApenas() {
    ...
    // 🔄 Atualizado para o Alerta Customizado
    if (!nome) { 
        mostrarAlertaCosmico("Identifique-se com seu nome de astronauta para registrar sua missão!", "🛸 Identificação Necessária"); 
        return; 
    }

    let validacaoGeral = true;
    Object.keys(definicaoGrupos).forEach(l => { if (escolhasDosGrupos[l].length !== 2) validacaoGeral = false; });

    // 🔄 Atualizado para o Alerta Customizado
    if (!validacaoGeral) { 
        mostrarAlertaCosmico("Telemetria incompleta! Você precisa escolher exatamente 2 classificados em cada um dos 12 grupos destacados.", "🛰️ Falha de Varredura"); 
        return; 
    }
    ...
    .then(response => {
        if (response.ok) {
            ...
            desenharPainelDeGrupos();
            carregarPalpitesDaPlanilha();
        } else { msg.style.color = "#ff3333"; msg.innerText = "❌ Falha ao salvar no servidor."; }
    })
    .catch(() => { msg.style.color = "#ff3333"; msg.innerText = "❌ Erro na conexão orbital."; })
    .finally(() => { btn.innerText = "Computar Classificados 🚀"; btn.disabled = false; });
}
...
function carregarPalpitesDaPlanilha() {
    ...
    fetch(urlPlanilha).then(r => r.json()).then(dados => {
        ...
        if (!dados || dados.length === 0) { cuerpoTabela.innerHTML = `<tr><td colspan="14">Nenhum palpite computado ainda.</td></tr>`; return; }
        ...
        if(window.twemoji) { twemoji.parse(cuerpoTabela); }
    }).catch(() => {});
}
...
function filtrarTabela() {
    ...
    for (let i = 0; i < linens.length; i++) {
        ...
        if (col) { linens[i].style.display = (col.textContent || col.innerText).toUpperCase().indexOf(filtro) > -1 ? "" : "none"; }
    }
}
...
async function consultarOraculo() {
    ...
    // 🔄 Atualizado para o Alerta Customizado
    if (!pergunta) { 
        mostrarAlertaCosmico("O Oráculo exige um questionamento! Digite uma pergunta antes de consultar os astros.", "🔮 Consulta Inválida"); 
        return; 
    }
    
    txt.innerText = "Decodificando resposta cósmica..."; box.style.display = "block";
    
    let apiKey = localStorage.getItem("zhavia_gemini_key");
    if (!apiKey) {
        apiKey = prompt("🔑 CONEXÃO SEGURA: Insira a Gemini API Key da Zhavia para ativar o Oráculo (Apenas uma vez por máquina):");
        if (!apiKey) return;
    ...
    try {
        const r = await fetch(url, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ contents: [{ parts: [{ text: `${instrucao}\n\nPergunta: ${pergunta}` }] }] }) });
        const d = await r.json(); txt.innerText = d.candidates[0].content.parts[0].text;
        document.getElementById("pergunta-oraculo").value = "";
    } catch { 
        // 🔄 Atualizado para o Alerta Customizado
        mostrarAlertaCosmico("Chave inválida ou problema de conexão. A memória local do Oráculo foi resetada.", "🛸 Falha do Sistema");
